...
import { mockProperties } from "../mock-properties.js"
import type { Property } from "../properties.js"
import { runWithLock } from "./mutex.js"
import type { CreatePropertyBody } from "./schemas.js"
...
function seedState(): MarketplaceState {
  return { properties: mockProperties.map((p) => ({ ...p })), unlocks: [] }
}
...
async function readDisk(): Promise<MarketplaceState | null> {
  ...
  try {
    ...
    if (!Array.isArray(parsed.properties) || !Array.isArray(parsed.unlocks)) return null
    return parsed
  } catch {
    return null
  }
...
async function loadInternal(): Promise<MarketplaceState> {
  if (!persistenceEnabled && volatileState) return volatileState
  const fromDisk = await readDisk()
  if (fromDisk) {
    if (!persistenceEnabled) volatileState = fromDisk
    return fromDisk
  }
...
export async function mutateMarketplaceState(mutator: (draft: MarketplaceState) => void): Promise<MarketplaceState> {
  return runWithLock(async () => {
    ...
    if (!persistenceEnabled) volatileState = next
    return next
  })
}
...
export async function appendUnlock(propertyId: string, sessionId: string): Promise<UnlockRecord> {
  const snapshot = await getMarketplaceState()
  if (!snapshot.properties.some((p) => p.id === propertyId)) throw new Error("PROPERTY_NOT_FOUND")

  let created: UnlockRecord | undefined
  await mutateMarketplaceState((draft) => {
    if (!draft.properties.some((p) => p.id === propertyId)) return
    created = {
  ...
  if (!created) throw new Error("PROPERTY_NOT_FOUND")
  return created
}
...
export async function hasUnlockForSession(propertyId: string, sessionId: string): Promise<boolean> {
  const state = await getMarketplaceState()
