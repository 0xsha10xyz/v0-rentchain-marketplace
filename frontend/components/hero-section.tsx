...
export function HeroSection({ t, onViewChange }: HeroSectionProps) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-primary via-primary to-primary/85">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(800px 420px at 20% 15%, rgba(255,255,255,0.16), transparent 60%), radial-gradient(700px 380px at 85% 35%, rgba(16,185,129,0.18), transparent 55%)",
        }}
      />

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-16 md:py-20">
        {/* Solana badge */}
        <div className="inline-flex items-center gap-2 bg-white/15 text-white rounded-full px-3 py-1 text-xs font-medium mb-6 ring-1 ring-white/15 backdrop-blur supports-[backdrop-filter]:bg-white/10">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            ...
            className="group flex items-center gap-3 bg-white text-primary font-semibold px-6 py-4 rounded-2xl hover:bg-white/95 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 ring-1 ring-white/10"
          >
          ...
          <button
            ...
            className="group flex items-center gap-3 bg-white/15 text-white font-semibold px-6 py-4 rounded-2xl hover:bg-white/25 transition-all border border-white/20 hover:-translate-y-0.5 backdrop-blur supports-[backdrop-filter]:bg-white/10"
          >
        ...
        <div className="mt-12 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {propertyTypes.map(({ icon: Icon, label, count }) => (
            <div key={label} className="flex items-center gap-3 bg-white/10 rounded-2xl px-4 py-3 ring-1 ring-white/10 backdrop-blur supports-[backdrop-filter]:bg-white/8">
