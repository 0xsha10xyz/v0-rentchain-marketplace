...
/** CAIP-2 ids must match `x402-solana` server `toCAIP2Network()`. */
...
/**
 * Ensures the client-signed `accepted` block matches this app's unlock policy before calling
 * the facilitator. The facilitator compares this object with the payload; it must be the
 * same object that was returned in the initial 402 `accepts[0]` (not a freshly built copy).
 */
